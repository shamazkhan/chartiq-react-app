...
TableViewBuilder.createToolbar = function (stx, config = {}) {
	...
	const _colHeaders = Object.assign({}, colHeaders);
...
TableViewBuilder.createTable = function (stx, config = {}) {
	...
	if (!stx.chart.highLowBars) {
		delete _colHeaders.open;
		delete _colHeaders.high;
		delete _colHeaders.low;
	}

	if (!tableView.viewAdditionalColumns) {
		for (let key in _colHeaders) {
			if (_colHeaders[key].cls) {
				delete _colHeaders[key];
			}
		}
	}

	const labels = Object.values(_colHeaders).map((item) => item.label);

	const seriesNames = getSeriesDataNames(stx);
	seriesNames.forEach((item) => (_colHeaders[item] = { label: item }));

	const studyNames = getStudyDataNames(stx);
	studyNames.forEach((item) => {
		if (!labels.includes(item)) _colHeaders[item] = { label: item };
	});

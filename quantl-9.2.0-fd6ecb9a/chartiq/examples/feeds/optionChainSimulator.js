...
const generateRandomInstrumentValues = (symbol, type, basis, factor) => {
	...
	const symbolParser = new RegExp("([A-Z]{1,5})(\\d?)(\\d{8})(C|P)(\\d{8})");
...
const updateData = (symbol, basis) => {
	...
	const updateValue = (value, isVol, isOI) => {
		let up = isVol || !!Math.round(CIQ.random());
		let lower = value / 1000; // 0.1% change
		let upper = value / 100; // 1% change
		let change = CIQ.random() * upper + lower;
		if (!up) change = -change;
		if (isVol) change = Math.round(3 * CIQ.random() + 1);
		if (isOI) change = Math.round(3 * CIQ.random() - 0.3);
...
const logTables = (newQuote) => {
	...
	for (let i in newQuote.optionChain) {
		...
		let strExp = i.replace(/(\d{8})[C|P](\d{8})/, "$1$2");
...
const updatePaginationCallback = function (symbol, params, cb) {
	return function callback(obj) {
		if (obj.error || !obj.quotes || obj.quotes.length === 0) return cb(obj);
		if (params.update || params.future) {
			...
			if (isMarketClosed(newestQuote.DT)) {
				...
				if (isDaily(params)) {
					if (
						...
						newestQuote.implVol = cachedData[symbol].implVol;
					} else {
						updateChain = true;
					}
				}
	...
	function callback(obj) {
		...
		while (quotesToPopulate.length) {
			...
			if (factor) {
				...
				quote.implVol = cachedData[symbol].implVol;
...
optionChainSimulator.fetchUpdateData = function (
	...
	const callback = updatePaginationCallback(symbol, params, cb);
	quoteFeedSimulator.fetchUpdateData(symbol, startDate, params, callback);
};
optionChainSimulator.fetchPaginationData = function (
	symbol,
	suggestedStartDate,
	endDate,
	params,
	...
	const callback = updatePaginationCallback(symbol, params, cb);
	quoteFeedSimulator.fetchPaginationData(
		symbol,
		suggestedStartDate,
		endDate,
		params,

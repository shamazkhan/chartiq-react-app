...
CIQ.MobileBridge.saveSignal = function (studyName, signalParams, editMode) {
	...
	try {
		...
		parsedParams.name = decodeURI(parsedParams.name);
		parsedParams.description = decodeURI(parsedParams.description);
		// save the signal
...
CIQ.MobileBridge.getActiveSignals = function () {
	...
	for (let key in stxx.layout.studies) {
		...
		if (study.signalData) {
			...
			if (this.isAndroid) slimSd.description = encodeURI(slimSd.description); // Android needs to be encoded or the special characters are not guaranteed to copy over, iOS works fine.
			slimSd.joiner = study.signalData.joiner;
			slimSd.signalName = study.signalData.name;
			if (this.isAndroid) slimSd.signalName = encodeURI(slimSd.signalName); // Android needs to be encoded or the special characters are not guaranteed to copy over, iOS works fine.
